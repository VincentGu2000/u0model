...
def initialize_weights(module):
    ...
    if isinstance(module, nn.Embedding):
        ...
        # Initialize 2D convolution layers
        nn.init.kaiming_normal_(module.weight, mode='fan_out', nonlinearity='relu')
        if module.bias is not None:
            nn.init.constant_(module.bias, 0)
    elif isinstance(module, nn.ConvTranspose2d):
        # Initialize transposed convolution layers
        nn.init.kaiming_normal_(module.weight, mode='fan_out', nonlinearity='relu')
        if module.bias is not None:
            nn.init.constant_(module.bias, 0)
        ...
        # Initialize 2D batch normalization
        if module.affine:
            nn.init.ones_(module.weight)
            nn.init.zeros_(module.bias)
...
class ArgsConfig:
    ...
    """Weight for target loss. Set to 0 to disable target loss backpropagation for ablation study."""
...
def main(config: ArgsConfig):
    """Main training function."""
    # ------------ step 1: load dataset ------------
    embodiment_tag = EmbodimentTag(config.embodiment_tag) # new_embodiment
    ...
    data_action_horizon = len(data_config_cls.action_indices) # 16
