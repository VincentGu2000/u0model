...
def calc_mse_for_single_trajectory(
    ...
    gt_target_pos_across_time = []
    pred_target_pos_across_time = []
    target_chunk = None
    ...
    for step_count in range(steps):
        ...
        if step_count % action_horizon == 0:
            ...
            action_chunk, target_chunk = policy.get_action(data_point)
        ...
        if target_chunk is not None:
            anchor_keys = ['target_pos']
            concat_pred_target_pos = np.concatenate(
                [np.atleast_1d(target_chunk[f"target_pos.{key}"]) for key in anchor_keys],
                axis=0,
            )
            pred_target_pos_across_time.append(concat_pred_target_pos)
            concat_gt_target_pos = np.concatenate(
                [data_point[f"target_pos.{key}"][0] for key in anchor_keys], axis=0
            )
            gt_target_pos_across_time.append(concat_gt_target_pos)
    ...
    has_target = len(gt_target_pos_across_time) > 0 and len(pred_target_pos_across_time) > 0
    if has_target:
        gt_target_pos_across_time = np.array(gt_target_pos_across_time)[:steps]
        pred_target_pos_across_time = np.array(pred_target_pos_across_time)[:steps]
        mse_target_pos = np.mean((gt_target_pos_across_time - pred_target_pos_across_time) ** 2)
        print("Unnormalized Target Position MSE across single traj:", mse_target_pos)
    else:
        mse_target_pos = 0.0
        print("Target loss is disabled, skipping target MSE calculation.")
    ...
    if plot or save_plot_path is not None:
        ...
        if has_target:
            info_target = {
                "gt_action_across_time": gt_target_pos_across_time,
                "pred_action_across_time": pred_target_pos_across_time,
                "modality_keys": modality_keys,
                "traj_id": traj_id,
                "mse": mse_target_pos,
                "action_dim": 6,
            ...
            plot_trajectory_target(info_target, save_plot_path)

    return mse, mse_target_pos
...
def plot_trajectory(
    ...
    # Smooth each column of pred_action_across_time
    # def smooth_with_moving_average(data, window_size=5):
    #     """
    #     Smooth the data using a moving average.
    #     :param data: numpy array to smooth
    #     :param window_size: size of the moving window
    #     :return: smoothed data
    #     """
    #     return np.convolve(data, np.ones(window_size)/window_size, mode='same')
    # pred_action_across_time = np.apply_along_axis(
    #     smooth_with_moving_average, axis=0, arr=pred_action_across_time, window_size=5
    # )
...
def plot_trajectory_target(
    ...
    title_text = f"target Analysis - ID: {traj_id}\nUnnormalized MSE: {mse:.6f}"
    ...
    for i, ax in enumerate(axes):
        ...
        ax.plot(gt_action_across_time[:, i], label="gt target", linewidth=2)
        ax.plot(pred_action_across_time[:, i], label="pred target", linewidth=2)
        ...
        ax.set_title(f"target Dimension {i}", fontsize=12, fontweight="bold", pad=10)
        ax.legend(loc="upper right", framealpha=0.9)
        ax.grid(True, alpha=0.3)
    ...
    if save_plot_path:
        target_plot_path = save_plot_path.replace('.png', '_target.png')
        print("saving target plot to", target_plot_path)
