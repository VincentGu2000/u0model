...
class U0botDataConfig(BaseDataConfig):
    video_keys = [
        "video.ego",
        "video.wrist",]
    state_keys = [
    ...
    def modality_config(self) -> dict[str, ModalityConfig]:
        ...
        target_pos_modality = ModalityConfig(
            delta_indices=self.observation_indices,
            modality_keys=self.target_pos_keys,
        )
        ...
        modality_configs = {
            ...
            "action": action_modality,
            "language": language_modality,
            "target_pos": target_pos_modality
        }
    ...
    def transform(self) -> ModalityTransform:
        transforms = [
            ...
            StateActionToTensor(apply_to=self.state_keys),
            StateActionSinCosTransform(apply_to=self.state_keys),
            # target_pos transforms
            StateActionToTensor(apply_to=self.target_pos_keys),
            StateActionTransform(
                apply_to=self.target_pos_keys,
                normalization_modes={key: "min_max" for key in self.target_pos_keys},
            ),
            # action transforms
            StateActionToTensor(apply_to=self.action_keys),
            StateActionTransform(
            ...
            ConcatTransform(
                ...
                state_concat_order=self.state_keys,
                action_concat_order=self.action_keys,
                # sensor_concat_order=self.sensor_keys,
                target_pos_concat_order=self.target_pos_keys,
            ),
            # model-specific transform
            GR00TTransform(
                state_horizon=len(self.observation_indices),
                action_horizon=len(self.action_indices),
                target_pos_horizon=len(self.observation_indices),
                max_state_dim=64,
                max_action_dim=32,
                max_target_pos_dim=6,
            ),
        ]
        return ComposedModalityTransform(transforms=transforms)
###########################################################################################
