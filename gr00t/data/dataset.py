...
class LeRobotSingleDataset(Dataset):
    ...
    def _get_metadata(self, embodiment_tag: EmbodimentTag) -> DatasetMetadata:
        ...
        for modality in [ "state", "action", "target_pos"]: #"sensor",
        ...
        dataset_statistics = {}
        for our_modality in [ "state", "action", "target_pos"]: #"sensor",
    ...
    def get_video(
        ...
        try:
            ...
            T = len(video_timestamp)  # Number of time steps
            # Derive the correct feature key from the video modality metadata
            original_key = self.lerobot_modality_meta.video[key].original_key
            if original_key is None:
                original_key = key
            H, W, C = self._lerobot_info_meta['features'][original_key]['shape']
            return np.zeros((T, H, W, C), dtype=np.uint8)   # Return all-zero image
    ...
    def get_data_by_modality(
        ...
        elif modality == "target_pos": #modality == "sensor" or 
            return self.get_state_or_action(trajectory_id, modality, key, base_index)
