...
class ConcatTransform(InvertibleModalityTransform):
    ...
    target_pos_concat_order: Optional[list[str]] = Field(
        default=None,
        description="Concatenation order for each target position modality. "
        "Format: ['target_pos.position', 'target_pos.velocity', ...].",
    )
    ...
    target_pos_dims: dict[str, int] = Field(
        default_factory=dict,
        description="The dimensions of the target position keys.",
    )
    ...
    def apply(self, data: dict) -> dict:
        ...
        if "target_pos" in grouped_keys:
            target_pos_keys = grouped_keys["target_pos"]
            assert self.target_pos_concat_order is not None, f"{self.target_pos_concat_order=}"
            assert all(
                item in target_pos_keys for item in self.target_pos_concat_order
            ), f"keys in target_pos_concat_order are misspecified, \n{target_pos_keys=}, \n{self.target_pos_concat_order=}"
            # Check the state dims
            for key in self.target_pos_concat_order:
                target_shapes = [self.target_pos_dims[key]]
                if self.is_rotation_key(key):
                    target_shapes.append(6)  # Allow for rotation_6d
                # if key in ["state.right_arm", "state.right_hand"]:
                target_shapes.append(self.target_pos_dims[key] * 2)  # Allow for sin-cos transform
                assert (
            ...
            data["target_pos"] = torch.cat(
                [data.pop(key) for key in self.target_pos_concat_order], dim=-1
            )  # [T, D_state]
    ...
    def unapply(self, data: dict) -> dict:
        ...
        if "target_pos" in data:
            assert self.target_pos_concat_order is not None, f"{self.target_pos_concat_order=}"
            start_dim = 0
            target_pos_tensor = data.pop("target_pos")
            for key in self.target_pos_concat_order:
                end_dim = start_dim + self.target_pos_dims[key]
                data[key] = target_pos_tensor[..., start_dim:end_dim]
                start_dim = end_dim
        return data
    ...
    def set_metadata(self, dataset_metadata: DatasetMetadata):
        ...
        if self.target_pos_concat_order is not None:
            for key in self.target_pos_concat_order:
                self.target_pos_dims[key] = self.get_state_action_dims(key)
