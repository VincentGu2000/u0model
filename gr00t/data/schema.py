...
# class LeRobotSensorMetadata(LeRobotStateActionMetadata):
#     """Metadata for a LeRobot state modality."""

#     original_key: Optional[str] = Field(
#         default="sensor",  # LeRobot convention for states
#         description="The original key of the sensor modality in the LeRobot dataset",
#     )

class LeRobotTargetPosMetadata(LeRobotStateActionMetadata):
    """Metadata for a LeRobot state modality."""

    original_key: Optional[str] = Field(
        default="target_pos",  # LeRobot convention for states
        description="The original key of the target_pos modality in the LeRobot dataset",
    )
...
class LeRobotModalityMetadata(BaseModel):
    ...
    # sensor: dict[str, LeRobotSensorMetadata] = Field(
    #     ...,
    #     description="The metadata for the sensor modality. The keys are the names of each split of the sensor vector.",
    # )
    target_pos: dict[str, LeRobotTargetPosMetadata] = Field(
        default_factory=dict,
        description="The metadata for the target_pos modality. The keys are the names of each split of the target_pos vector.",
    )
    ...
    def get_key_meta(self, key: str) -> LeRobotModalityField:
        ...
        #     if subkey not in self.sensor:
        #         raise ValueError(
        #             f"Key: {key}, sensor key {subkey} not found in metadata, available sensor keys: {self.sensor.keys()}"
        #         )
        #     return self.sensor[subkey]
        elif modality == "target_pos":
            if subkey not in self.target_pos:
                raise ValueError(
                    f"Key: {key}, target_pos key {subkey} not found in metadata, available target_pos keys: {self.target_pos.keys()}"
                )
            return self.target_pos[subkey]
...
class DatasetStatistics(BaseModel):
    ...
    # sensor: dict[str, DatasetStatisticalValues] = Field(..., description="Statistics of the sensor")
    target_pos: dict[str, DatasetStatisticalValues] = Field(default_factory=dict, description="Statistics of the target_pos")
...
class DatasetModalities(BaseModel):
    ...
    # sensor: dict[str, StateActionMetadata] = Field(..., description="Metadata of the sensor")
    target_pos: dict[str, StateActionMetadata] = Field(default_factory=dict, description="Metadata of the target_pos")
