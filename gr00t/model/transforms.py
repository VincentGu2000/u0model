...
class GR00TTransform(InvertibleModalityTransform):
    ...
    max_state_dim: int
    max_action_dim: int
    max_target_pos_dim: int = 0
    state_horizon: int
    action_horizon: int
    target_pos_horizon: int = 0
    max_length: int = 512
    ...
    def _prepare_target_pos(self, data: dict):
        """
        Gathers final state from data['state'], then pads to max_state_dim.
        Return (state, state_mask, n_state_tokens).
        """
        if "target_pos" not in data:
            target_pos = np.zeros((self.target_pos_horizon, self.max_target_pos_dim))
            target_pos_mask = np.zeros((self.target_pos_horizon, self.max_target_pos_dim), dtype=bool)
            n_target_pos_tokens = self.target_pos_horizon
            return target_pos, target_pos_mask, n_target_pos_tokens

        target_pos = data["target_pos"]
        assert target_pos.shape[0] == self.target_pos_horizon, f"{target_pos.shape=}, {self.target_pos_horizon=}"

        n_target_pos_dims = target_pos.shape[-1]

        # Instead of asserting, just take the first max_state_dim dimensions if needed
        if n_target_pos_dims > self.max_target_pos_dim:
            target_pos = target_pos[:, : self.max_target_pos_dim]
            n_target_pos_dims = self.max_target_pos_dim
        else:
            # Pad up to max_state_dim if smaller
            target_pos = np.pad(target_pos, ((0, 0), (0, self.max_target_pos_dim - n_target_pos_dims)), "constant")

        # Create mask for real state dims
        target_pos_mask = np.zeros_like(target_pos).astype(bool)
        target_pos_mask[:, :n_target_pos_dims] = True

        # We only have 1 "proprio" token to represent the entire state
        n_target_pos_tokens = target_pos.shape[0]
        return target_pos, target_pos_mask, n_target_pos_tokens
    ...
    def apply_single(self, data: dict) -> dict:
        ...
        if self.training:
            ...
            target_pos, target_pos_mask, _ = self._prepare_target_pos(data)
            transformed_data["action"] = actions
            transformed_data["action_mask"] = actions_mask
            transformed_data["target_pos"] = target_pos
            transformed_data["target_pos_mask"] = target_pos_mask
