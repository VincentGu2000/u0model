...
class Gr00tPolicy(BasePolicy):
    ...
    def get_action(self, observations: Dict[str, Any]) -> Dict[str, Any]:
        ...
        normalized_action, normalized_target = self._get_action_from_normalized_input(normalized_input)
        unnormalized_action = self._get_unnormalized_action(normalized_action)
        unnormalized_target = self._get_unnormalized_target(normalized_target) if normalized_target is not None else None

        if not is_batch:
            unnormalized_action = squeeze_dict_values(unnormalized_action)
            if unnormalized_target is not None:
                unnormalized_target = squeeze_dict_values(unnormalized_target)
        return unnormalized_action, unnormalized_target
    ...
    def _get_action_from_normalized_input(self, normalized_input: Dict[str, Any]):
        ...
        normalized_target = model_pred["target_pred"].float() if "target_pred" in model_pred else None
    ...
    def _get_unnormalized_target(self, normalized_target: torch.Tensor) -> Dict[str, Any]:
        return self.unapply_transforms({"target_pos": normalized_target.cpu()})
